...
import com.blue.corelib.utils.dp
...
class CommonShapeButton @JvmOverloads constructor(
    ...
    fun setStrokeColor(color: Int, strokeWidth: Int) {
        mStrokeColor = color
    ...
    fun setStartEndColor(startColor: Int, endColor: Int, orientation: Int = 0) {
     ...
     * 触控增加区域
     */
    private var mTouchAdd = 0
    private var mTouchAddBottom = 0
    private var mTouchAddLeft = 0
    private var mTouchAddRight = 0
    private var mTouchAddTop = 0

    //避免 DrawAllocation warning
    private var mTouchRect: Rect? = null
    private val mTouchDelegate: TouchDelegate by lazy { TouchDelegate(mTouchRect, this) }
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.CommonShapeButton).apply {
            ...
            mPressedColor =
                getColor(R.styleable.CommonShapeButton_csb_pressedColor, 0xFF666666.toInt())
            mStrokeColor = getColor(R.styleable.CommonShapeButton_csb_strokeColor, 0)
            mStrokeWidth = getDimensionPixelOffset(R.styleable.CommonShapeButton_csb_strokeWidth, 0)
            ...
            mDrawableWidth =
                getDimensionPixelOffset(R.styleable.CommonShapeButton_csb_drawableWidth, 0)
            mDrawableHeight =
                getDimensionPixelOffset(R.styleable.CommonShapeButton_csb_drawableHeight, 0)
            ...
            mTouchAdd = getDimensionPixelSize(R.styleable.CommonShapeButton_csb_touchAdd, 0)
            mTouchAddBottom =
                getDimensionPixelSize(R.styleable.CommonShapeButton_csb_touchAddBottom, mTouchAdd)
            mTouchAddLeft =
                getDimensionPixelSize(R.styleable.CommonShapeButton_csb_touchAddLeft, mTouchAdd)
            mTouchAddRight =
                getDimensionPixelSize(R.styleable.CommonShapeButton_csb_touchAddRight, mTouchAdd)
            mTouchAddTop =
                getDimensionPixelSize(R.styleable.CommonShapeButton_csb_touchAddTop, mTouchAdd)
            centerText = getBoolean(R.styleable.CommonShapeButton_csb_centerText, true)
            if (mTouchAddBottom > 0 && mTouchAddLeft > 0 && mTouchAddRight > 0 && mTouchAddTop > 0) {
                mTouchRect = Rect()
            }
            recycle()
        }
    ...
    fun setBackgroundFillColor(@ColorInt color: Int) {
        mFillColor = color
        updateBackground()
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
            ...
            compoundDrawables?.let {
                ...
                drawable?.let {
                    ...
                    if (mDrawableWidth > 0 && mDrawableHeight > 0) {
                        it.setBounds(0, 0, mDrawableWidth, mDrawableHeight)
                        when (mDrawablePosition) {
                            0 -> setCompoundDrawables(it, null, null, null)
                            1 -> setCompoundDrawables(null, it, null, null)
                            2 -> setCompoundDrawables(null, null, it, null)
                            3 -> setCompoundDrawables(null, null, null, it)
                        }
                    }
                    // 图片间距
                    ...
                    when (mDrawablePosition) {
                        ...
                        0, 2 -> {
                            // 图片宽度
                            val drawableWidth = if (mDrawableWidth > 0) {
                                mDrawableWidth
                            } else {
                                it.intrinsicWidth
                            }
                        ...
                        1, 3 -> {
                            // 图片高度
                            val drawableHeight = if (mDrawableHeight > 0) {
                                mDrawableHeight
                            } else {
                                it.intrinsicHeight
                            }
        ...
        if (changed) {
            mTouchRect?.apply {
                this.left = left - mTouchAddLeft.dp
                this.top = top - mTouchAddTop.dp
                this.right = right + mTouchAddRight.dp
                this.bottom = bottom + mTouchAddBottom.dp
                val parent = this@CommonShapeButton.parent as View
